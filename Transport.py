...
class VirtualTransport():
    def __init__(self):
        ...
        #ifs = ioctl(f, TUNSETIFF, struct.pack("16sH", "tun0", TUNMODE))
        ioctl(f, TUNSETIFF, struct.pack("16sH", VIRT_IFACE_NAME, TUNMODE))
        ...
        #os.system("sudo ifconfig tun0 %s/24 mtu 1400" % node_ip)            # !!! MTU value is fixed for now. !!!
    ...
    # Returns a list of ip addresses assigned to the virtual interface (in a form of: [<ipv4 address>, <ipv6 address1>,  <ipv6 address2>,  <ipv6 addressN>])
...
class RawTransport():
    def __init__(self, dev, node_mac, topology_neighbors):
        ...
        self.send_socket.bind((dev, 0x7777))            # Type 0x7777 corresponds to the chosen "protocol_type" in our custom ethernet frame
    ...
    def gen_dsr_header(self, dsr_header):
        dsr_bin_string = struct.pack(dsr_header.header_format, dsr_header.type, dsr_header.length, 
                                     self.mac2int(dsr_header.src_mac), self.mac2int(dsr_header.dst_mac), self.mac2int(dsr_header.tx_mac))
    ...
    def recv_data(self):
        while self.running:
            ...
            if src_mac in self.topology_neighbors:
                ...
                dsr_header_obj = Messages.DsrHeader()
                dsr_header_str = data[14:(14 + dsr_header_obj.length)]                  # Skip the Ethernet header, which length is equaled to 14
                dsr_data = struct.unpack(dsr_header_obj.header_format, dsr_header_str)  # Dsr data in a form (type, length, src_ip, dst_ip)
                
                # Put the received dsr_header values into the object
                dsr_header_obj.type      = dsr_data[0]
                dsr_header_obj.length    = dsr_data[1]
                ...
                print "!!! THIS IS MY OWN MAC !!! %s" % src_mac
